...
class CVATApplication extends React.PureComponent<CVATAppProps & RouteComponentProps, CVATAppState> {
    private t = (key: string): string => i18n.t(key);
    ...
    public componentDidMount(): void {
        ...
        core.server.healthCheck(
            ...
            .catch(() => {
                ...
                Modal.error({
                    title: this.t('errors.cannotConnectToServer'),
    ...
    <Text>
        ...
        if (showPlatformNotification()) {
            ...
            Modal.warning({
                title: this.t('errors.unsupportedPlatformDetected'),
            ...
            stopNotifications(false);
            Modal.warning({
                title: this.t('errors.unsupportedFeaturesDetected'),
        ...
        function showError(title: string, _error: Error, shouldLog?: boolean, className?: string): void {
            ...
            notification.error({
                ...
                description: errorLength > appConfig.MAXIMUM_NOTIFICATION_MESSAGE_LENGTH ?
                    this.t('errors.openBrowserConsoleForDetails') : <CVATMarkdown history={history}>{error}</CVATMarkdown>,
            });
        ...
        if (healthIinitialized && !backendIsHealthy) {
            return (
                <Space align='center' direction='vertical' className='cvat-spinner'>
                    ...
                    {this.t('loading.cannotConnect')}
                </Space>
            );
        }

        return (
            <Spin size='large' fullscreen className='cvat-spinner' tip={this.t('loading.connecting')} />
        );
    }
}
