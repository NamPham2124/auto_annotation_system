...
function MemberItem(props: Readonly<Props>): JSX.Element {
    ...
    const onUpdateMembershipRole = (newRole: string): void => {
        ...
        dispatch(makeBulkOperationAsync(
            ...
            (m, idx, total) => t('organization.updatingRole', { username: m.user.username, current: idx + 1, total }),
            fetchMembers,
        ));
    };
            ...
            triggerElement={(
                ...
                >
                    ...
                    <Col span={8} className='cvat-organization-member-item-dates'>
                        {invitation ? (
                            <Text type='secondary'>
                                {t('organization.invited', { time: dayjs(invitation.createdDate).fromNow() })}
                                {invitation.owner && ` ${t('organization.invitedBy', { username: invitation.owner.username })}`}
                            </Text>
                        ) : null}
                        {joinedDate ? <Text type='secondary'>{t('organization.joined', { time: dayjs(joinedDate).fromNow() })}</Text> : <Text type='secondary'>{t('organization.invitationPending')}</Text>}
                    </Col>
