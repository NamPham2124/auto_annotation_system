        ...
        const modal = Modal.confirm({
            ...
            content: (
                <div className='cvat-remove-organization-submit'>
                    <Text type='warning'>
                        {t('organization.toRemoveOrganization')}
                    </Text>
                    <Input
                        onChange={
            ...
            <Row justify='space-between'>
                <Col span={24}>
                    <div className='cvat-organization-top-bar-descriptions'>
                        <Row justify='space-between'>
                            <Col>
                                <Text>
                                    <Text className='cvat-title'>{`${t('organization.title')}: ${slug} `}</Text>
                                </Text>
                            </Col>
                            <Col>
                                <Dropdown
                                        ...
                                        items: [
                                            {
                                                ...
                                                label: <Link to='/organization/webhooks'>{t('organization.setupWebhooks')}</Link>,
                                            },
                                            ...(owner && userID === owner.id ? [{
                                                ...
                                                onClick: onRemove,
                                                label: t('organization.removeOrganization'),
                                            }] : []),
                                        ],
                                ...
                                >
                                    <Button size='middle' className='cvat-organization-page-actions-button'>
                                        <Text className='cvat-text-color'>{t('organization.actions')}</Text>
                        ...
                        {!editingDescription ? (
                            <span style={{ display: 'grid' }}>
                                {(description || t('organization.addDescription')).split('\n').map((val: string, idx: number) => (
                ...
                <Col span={12}>
                    <div className='cvat-organization-top-bar-contacts'>
                        <div>
                            <PhoneOutlined />
                            { !contact.phoneNumber ? <Text type='secondary'>{t('organization.addPhoneNumber')}</Text> : null }
                            <Text
                                type='secondary'
                                editable={{
                                    onStart() {
                        ...
                        <div>
                            <MailOutlined />
                            { !contact.email ? <Text type='secondary'>{t('organization.addEmail')}</Text> : null }
                            <Text
                                type='secondary'
                                editable={{
                                    onStart() {
                        ...
                        <div>
                            <EnvironmentOutlined />
                            { !contact.location ? <Text type='secondary'>{t('organization.addLocation')}</Text> : null }
                            <Text
                                type='secondary'
                                editable={{
                                    onStart() {
                        ...
                        <Text type='secondary'>{t('organization.created', { date: dayjs(createdDate).format('MMMM Do YYYY') })}</Text>
                        <Text type='secondary'>{t('organization.updated', { time: dayjs(updatedDate).fromNow() })}</Text>
                    </div>
                </Col>
                            ...
                            <Button
                                ...
                                onClick={() => {
                                    Modal.confirm({
                                        ...
                                        content: (
                                            <>
                                                <Text>{t('organization.pleaseConfirmLeaving')}</Text>
                                                <Text strong>{` ${organizationInstance.slug}`}</Text>
                                                <Text>{t('organization.youWillNotHaveAccess')}</Text>
                                            </>
                                        ),
                                        okText: t('organization.leave'),
                                        okButtonProps: {
                                            danger: true,
                                        },
                                    });
            ...
            <Row className='cvat-organization-page-filters-wrapper' justify='space-between'>
                ...
                <Col>
                    <SortingComponent
                        ...
                        sortingFields={[t('organization.user'), t('organization.role')]}
