...
function checkFiles(files: RemoteFile[], type: SupportedShareTypes, baseError: string, t: (...args: any[]) => string): string {
    ...
    if (erroredFiles.length !== 0) {
        ...
        return extensionList.length ? `${baseError} ${t('tasks.foundUnsupportedTypes', { types: extensionList.join(', ') })}` : baseError;
    }
    return '';
}

function validateRemoteFiles(remoteFiles: RemoteFile[], many: boolean, t: (...args: any[]) => string): string {
    ...
    if (!many && excludedManifests.length > 1) {
        ...
        uploadFileErrorMessage = checkFiles(
            ...
            t('tasks.uploadArchiveErrorMulti'),
            t,
        );
    }
...
class CreateTaskContent extends React.PureComponent<Props & RouteComponentProps, State> {
        ...
        try {
            if (!many && length > 1) {
                ...
                while (index < length) {
                    ...
                    if (!isImageFile) {
                        uploadFileErrorMessage = t('tasks.uploadArchiveErrorOne');
                        break;
                    }
                    index++;
                }
                ...
                while (index < length) {
                    ...
                    if (!isVideoFile) {
                        uploadFileErrorMessage = t('tasks.uploadArchiveErrorMulti');
                        break;
                    }
                    index++;
                }
    ...
    private renderProjectBlock(): JSX.Element {
        ...
        return (
            <>
                <Col span={24}>
                    <Text className='cvat-text-color'>{t('tasks.project')}</Text>
                </Col>
                <Col span={24}>
    ...
    private renderSubsetBlock(): JSX.Element | null {
        ...
        if (projectId !== null) {
            return (
                <>
                    <Col span={24}>
                        <Text className='cvat-text-color'>{t('tasks.subsetField')}</Text>
                    </Col>
                    <Col span={24}>
                        <ProjectSubsetField
                            value={subset}
    ...
    private renderLabelsBlock(): JSX.Element {
        ...
        if (projectId) {
            return (
                <>
                    <Col span={24}>
                        <Text className='cvat-text-color'>{t('tasks.taskDetails.labels')}</Text>
                    </Col>
                    <Col span={24}>
                        <Text type='secondary'>{t('tasks.projectLabelsWillBeUsed')}</Text>
                    </Col>
                </>
            );
        }
        ...
        return (
            <>
                <Col span={24}>
                    ...
                    <Text className='cvat-text-color'>{t('tasks.selectFiles')}</Text>
        ...
        return (
            <Col span={24}>
                <Collapse
                    ...
                    items={[{
                        key: '1',
                        label: <Text className='cvat-title'>{t('tasks.advancedConfiguration')}</Text>,
                        children: (
        ...
        return (
            <Col span={24}>
                <Collapse
                    ...
                    items={[{
                        key: '1',
                        label: <Text className='cvat-title'>{t('tasks.quality')}</Text>,
                        children: (
    ...
    public render(): JSX.Element {
        ...
        return (
            <Row justify='start' align='middle' className='cvat-create-task-content'>
                <Col span={24}>
                    <Text className='cvat-title'>{t('tasks.basicConfiguration')}</Text>
                </Col>
...
export default withRouter(withTranslation()(CreateTaskContent as any));
