...
function ProjectActionsComponent(props: Readonly<Props>): JSX.Element {
    ...
    const onUpdateProjectAssignee = useCallback((assignee: User | null) => {
        ...
        if (onUpdateProject) {
            ...
            dispatch(makeBulkOperationAsync(
                ...
                (project, idx, total) => t('projects.updatingAssignee', {
                    id: project.id,
                    current: idx + 1,
                    total,
                }),
    ...
    const onUpdateProjectOrganization = useCallback((newOrganization: Organization | null) => {
        ...
        const updateBulk = () => {
            dispatch(makeBulkOperationAsync(
                ...
                (project, idx, total) => t('projects.updatingOrganization', {
                    id: project.id,
                    current: idx + 1,
                    total,
                }),
    ...
    const onDeleteProject = useCallback((): void => {
        ...
        Modal.confirm({
            title: isBulkMode ?
                t('projects.deleteSelectedProjects', { count: projectsToDelete.length }) :
                t('projects.deleteProjectConfirm', { id: projectInstance.id }),
            content: isBulkMode ?
                t('projects.deleteSelectedProjectsConfirm') :
                t('projects.deleteProjectDataConfirm'),
            ...
            onOk: () => {
                dispatch(makeBulkOperationAsync(
                    ...
                    (project, idx, total) => t('projects.deletingProject', {
                        id: project.id,
                        current: idx + 1,
                        total,
                    }),
            ...
            okText: isBulkMode ? t('projects.deleteSelected') : t('projects.delete'),
        });
