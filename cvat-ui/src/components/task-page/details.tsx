...
type Props = DispatchToProps & StateToProps & OwnProps & { t: (key: string, options?: Record<string, unknown>) => string };
...
class DetailsComponent extends React.PureComponent<Props, State> {
                ...
                <Row className='cvat-task-details-user-block' justify='space-between' align='middle'>
                    <Col span={12}>
                        {owner && (
                            <div>
                                <Text type='secondary'>
                                    {t('tasks.taskDetails.taskCreatedBy', { id: taskInstance.id, owner, created })}
                                </Text>
                            </div>
                        )}
                    ...
                    <Col>
                        <Text type='secondary'>{t('tasks.assignedTo')}</Text>
                        {assigneeSelect}
                    </Col>
                </Row>
    ...
    private renderSubsetField(): JSX.Element {
        ...
        return (
            <Row>
                <Col span={24}>
                    <Text className='cvat-text-color'>{t('tasks.subset')}:</Text>
                </Col>
                <Col span={24}>
                    <ProjectSubsetField
                        value={subset}
...
export default withTranslation()(connect(mapStateToProps, mapDispatchToProps)(DetailsComponent) as any);
