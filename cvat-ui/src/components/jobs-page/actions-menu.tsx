...
function JobActionsComponent(
    ...
    const onMergeConsensusJob = useCallback(() => {
        if (consensusJobsPresent && jobInstance.parentJobId === null) {
            Modal.confirm({
                title: t('jobs.mergeConsensusJobConfirm'),
                content: t('jobs.mergeConsensusJobContent'),
                ...
                okText: t('jobs.mergeConsensusJob'),
            });
        }
    }, [consensusJobsPresent, jobInstance, t]);
    ...
    const onDeleteJob = useCallback(() => {
        ...
        Modal.confirm({
            title: isBulk ?
                t('jobs.deleteSelectedJobs', { count: jobsToDelete.length }) :
                t('jobs.deleteJobConfirm', { id: jobInstance.id }),
            content: isBulk ?
                t('jobs.deleteSelectedJobsConfirm') :
                t('jobs.deleteJobDataConfirm'),
            ...
            onOk: () => {
                setTimeout(() => {
                    dispatch(makeBulkOperationAsync(
                        ...
                        (job, idx, total) => `${t('jobs.delete')} #${job.id} (${idx + 1}/${total})`,
                    ));
                }, 0);
            },
            okButtonProps: {
            ...
            okText: isBulk ? t('jobs.deleteSelectedJobs', { count: jobsToDelete.length }) : t('jobs.delete'),
        });
    }, [jobInstance, allJobs, selectedIds, dispatch, t]);
